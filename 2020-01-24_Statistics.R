...
#Si la correlación es 0 indica que la relación lineal entre x e y es nula.
#Si es 1 la relación lineal es directa
#Si es -1 la relación lineal es indirecta
...
#normalización estándar: se intenta que todos los datos queden en una escala entre min y max
...
#al poner two sided vemos si la media tiene ese valor o no, si no podriamos poner greater
...
sd(tips$tip[which(tips$sex=="Male")])
sd(tips$tip[which(tips$sex=="Female")])
#son aprox. iguales
...
#los chicos y las chicas tienen las mismas propinas
...
#las propinas son iguales todos los días (en media)?
#Ho: alguna muestra es diferente?
#Ha: todas son iguales
...
#esto es en variables continuas
...
#esto es en variables discretas
