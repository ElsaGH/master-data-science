...
hist(df_temp$price,prob=TRUE) #histograma de frecuencias relativas
lines(density(df_temp$price))

lillie.test(df_temp$price) #test de normalidad
...
#sapply: le pasas un dataset y aplica la función por columnas a todo el dataset
#apply: le pasas un dataset y aplica funcion por filas o columnas (0 es fila y 1 columna)
#lapply: le pasas una lista y aplica la funcion a la lista
#rapply: le pasas una lista y aplica la función a la lista y también a la lista de la lista
...
aggregate(data_scrap[,"price"], by = list(localidad=data_scrap$level5), FUN = function(x){round(max(x,na.rm=T),2)})
...
my_map <- leaflet() %>%
  ...
  addCircleMarkers(lng=data_scrap$longitude,lat=data_scrap$latitude, radius = 5, 
                   popup=data_scrap$id_realEstates, color = pal(data_scrap$level5))
my_map
...
summary(lm_model)
#adjusted R-squared depende de las variables, el multiple R-squared no
...
#los valores de abajo nos modifican la recta, hay que eliminar esos ceros

data_scrap<-data_scrap[data_scrap$price>0,]

#normaliza el precio y repítelo
...
na.omit(data_scrap) #elimina todo pq hay una columna que es todo NA
...
#vamos a eliminar outliers por localidad
...
#aceptamos Ho, el precio por m2 es igual

shapiro.test(df_temp$pricem2)
ansari.test(pricem2 ~level5,df_temp)
...
#Ho: sdx/sdy = 1
#puedo aplicar el two sample t-test con varianzas iguales
